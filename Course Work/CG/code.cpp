...
void myViewer::set_position()
{
    ...
    if (v_beta >= 360)
        ...
        v_beta += 360;
    pos[0] = (v_radius * cos(convert *  v_beta)) * cos(convert * v_alpha);
    pos[1] = v_radius * sin(convert * v_beta);
    pos[2] = (v_radius * cos(convert * v_beta)) * sin(convert * v_alpha);
}
...
void myViewer::reset_view(double alpha, double beta, double radius, double min_radius)
{
    v_alpha = alpha; v_beta = beta; v_radius = radius; v_min_radius = min_radius;
