...
public class EditPharmacistPanel extends JPanel implements ActionListener{
	...
	public EditPharmacistPanel(AdminController _controller) {
		setLayout(null);
		...
		JLabel lblAddCotor = new JLabel("Edit Pharmacist");
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getActionCommand().toString().equals("Save"))
		{
			PharmacistModel pm=pharmacistobjects.get(comboBox.getSelectedIndex());
			...
			pm.setDepartment_code(Integer.parseInt(department_idField.getText()));
			pm.setZip(zipField.getText());
			
			pm.setPhone(phoneField.getText());
			
			pm.setEmail(emailField.getText());
			pm.setAdmin_id(Integer.parseInt(admin_idField.getText()));
			pm.setAdmin_id(Integer.parseInt(pharmacist_idField.getText()));
			...
			try {
		      System.out.println(pm.getFirst_name());
				pharmacistcontroller.updatePharmacistRecord(pm);
			} catch (Exception exe) {
				exe.printStackTrace();
			}
		}
