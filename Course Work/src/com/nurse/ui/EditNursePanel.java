...
public class EditNursePanel extends JPanel implements ActionListener{
	...
	private JTextField nurse_idField;
	...
	private NurseController nursecontroller=null;
	private ArrayList<NurseModel> nurseobjects=null;
	
	public EditNursePanel(AdminController _controller) {
		setLayout(null);
		controller = _controller;
		if(controller.getNurseController()==null)
		{
			nursecontroller=new NurseController();
			controller.setNurseController(nursecontroller);

		}
		else
		{
		nursecontroller=controller.getNurseController();
		}

		nurseobjects=nursecontroller.getNurseObjects();
		...
		JLabel lblAddCotor = new JLabel("Edit Nurse");
		...
		JLabel lblNurseName = new JLabel("First Name");
		lblNurseName.setHorizontalAlignment(SwingConstants.LEFT);
		lblNurseName.setFont(new Font("Times New Roman", Font.PLAIN, 16));
		lblNurseName.setBounds(48, 137, 102, 27);
		add(lblNurseName);
		...
		JLabel lblDoctorId = new JLabel("Nurse");
		...
		JLabel lblDocotorId = new JLabel("Nurse Id");
		...
		nurse_idField = new JTextField();
		nurse_idField.setColumns(10);
		nurse_idField.setBounds(169, 324, 126, 22);
		add(nurse_idField);
		
		comboBox = new JComboBox(nursecontroller.getNurseList().toArray());
		comboBox.setBounds(167, 104, 263, 20);
		add(comboBox);
		comboBox.addActionListener(this);
		firstNameField.setText(nurseobjects.get(comboBox.getSelectedIndex()).getFirst_name());
		middleNameField.setText(nurseobjects.get(comboBox.getSelectedIndex()).getMiddle_name());
		lastNameField.setText(nurseobjects.get(comboBox.getSelectedIndex()).getLast_name());
		streetField.setText(nurseobjects.get(comboBox.getSelectedIndex()).getStreet());
		cityField.setText(nurseobjects.get(comboBox.getSelectedIndex()).getCity());
		department_idField.setText(Integer.toString(nurseobjects.get(comboBox.getSelectedIndex()).getDepartment_code()));
		zipField.setText(nurseobjects.get(comboBox.getSelectedIndex()).getZip());
		phoneField.setText(nurseobjects.get(comboBox.getSelectedIndex()).getPhone());
		emailField.setText(nurseobjects.get(comboBox.getSelectedIndex()).getEmail());
		admin_idField.setText(Integer.toString(nurseobjects.get(comboBox.getSelectedIndex()).getAdmin_id()));
		nurse_idField.setText(Integer.toString(nurseobjects.get(comboBox.getSelectedIndex()).getNurse_id()));
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getActionCommand().toString().equals("Save"))
		{
			NurseModel nm=nurseobjects.get(comboBox.getSelectedIndex());
			nm.setFirst_name(firstNameField.getText());
			
			nm.setMiddle_name(middleNameField.getText());
			
			nm.setLast_name(lastNameField.getText());
			
			nm.setStreet(streetField.getText());
			
			nm.setCity(cityField.getText());
			
			nm.setDepartment_code(Integer.parseInt(department_idField.getText()));
			nm.setZip(zipField.getText());
			
			nm.setPhone(phoneField.getText());
			
			nm.setEmail(emailField.getText());
			nm.setAdmin_id(Integer.parseInt(admin_idField.getText()));
			nm.setAdmin_id(Integer.parseInt(nurse_idField.getText()));
			...
			try {
		      System.out.println(nm.getFirst_name());
				nursecontroller.updateNurseRecord(nm);
			} catch (Exception exe) {
				exe.printStackTrace();
			}
		}
